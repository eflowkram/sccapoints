...
./clubpoints.py -u <index-url>        # scrape a whole season from its index
./clubpoints.py -u <url-or-file>      # or one results page / local file
...
Cell *values* drift too, not just columns. Some seasons write two times into one
`Total` cell, `"scored / other"` — raw best and indexed equivalent. Which leads
depends on the class, but it is always the value the class is ranked on, so
`scoring_time()` takes the part before the slash. Unhandled, this silently
skipped every row in the file and whole events vanished from the standings.

The Axware HTML comment (`Generated by: AXWare TS ... -2024-05-01`) is the
**software build date**, not the event date. Do not use it.

Expect more of this. Axware's output format changes between seasons, so when
adding support for a new one, capture the file into `tests/fixtures/real/` and
watch the warning count — a clean run that drops rows is the failure mode to
look for, not a crash.

`-u` accepts a season index as well as a results page. `result_links()` treats a
page as an index when it links to `.htm` files whose names carry a MM-DD-YYYY
date, and returns them oldest first. Keep the single-page and local-file paths
working — they are the fallback when a region's index is laid out differently.
